...
function bigmap(){
	...
	lang.forEach(x => {
		...
		if (document.getElementById('b_feature').checked){
			...
			const wants2 = document.getElementById('b_feature_text2').value.toLowerCase();
		...
		if (conditional){
			...
			if (x.features){
				if (commonf === true){
					...
					for (const property in x.features){
		...
		// bigmap
		const coords = coord2px(x.coords);
		...
		newpoint.alt = x.name;
		const familylist = x.families.length ? x.families.map(proper).join(', ') : 'Isolate';
		...
		if (typeof x.sources !== 'undefined'){
			sources = '';
			for (const sourcen in range(x.sources.length)){
				// console.log(sourcen);
